...
int main(){
    ...
    cout << endl << endl << endl << ">\n>> CRACKED THE PASSWORD! >>\n>" << endl <<  endl <<"The password : " << endl<<crackPassword(password) << endl;
    cout << "The number of attempts : " << attempt << endl;
    return 0;
}

string crackPassword(string pass){
    int digit[8],alphabetSet=1,passwordLength=1;

    
    string test,alphabet = "1337 also daktari is pro";//initialisation
    while(1){
        
        switch(passwordLength){ //switch statement for password lenght
            case 1:
                while(alphabetSet<4){
                    switch(alphabetSet){
        ...
        cout
        << "*** Password length is not " << passwordLength <<  ". Increasing password length! ***";
        cout  << endl << "*" << endl << endl;
        passwordLength++;
    }
}
